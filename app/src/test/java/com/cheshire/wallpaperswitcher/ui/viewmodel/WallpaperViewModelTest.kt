...
class WallpaperViewModelTest {
    ...
    private fun mockDirectoryLastModified(uri: Uri, lastModified: Long) {
    ...
    @Test
    fun `cache is valid when folder and modification date match`() = runTest {
        val folderUri = baseTestUri
    ...
    @Test
    fun `cache is invalid if folder uri changes`() = runTest {
    ...
    @Test
    fun `cache is invalid if last modified date changes`() = runTest {
        val folderUri = baseTestUri
        repository.saveFolderUri(folderUri)
